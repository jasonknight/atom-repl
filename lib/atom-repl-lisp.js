...
export default class AtomReplLisp {
    ...
    createREPL() {
        var path = atom.config.get('atom-repl.lispPath');
        if ( path == "none" ) {
            console.log("You need to edit the lispPath setting");
            return;
        }
        var self = this;
        AtomReplUtils.maybeREPLExists(self,"atom-repl-lisp");
    ...
    setupREPL() {
        ...
        var tmpFile = tempy.file({extension: 'lisp'});
    ...
    destroyREPLS() {
        AtomReplUtils.destroyREPLS("atom-repl-lisp");
    }
