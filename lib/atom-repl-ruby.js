...
export default class AtomReplRuby {
    ...
    createREPL() {
        var path = atom.config.get('atom-repl.rubyPath');
        if ( path == "none" ) {
            console.log("You need to edit the rubyPath setting");
            return;
        }
        var self = this;
        AtomReplUtils.maybeREPLExists(self,"atom-repl-ruby");
    ...
    setupREPL() {
        ...
        var tmpFile = tempy.file({extension: 'rb'});
    ...
    destroyREPLS() {
        AtomReplUtils.destroyREPLS("atom-repl-ruby");
    }
